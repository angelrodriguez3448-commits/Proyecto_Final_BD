...
def ventana_agregar_cita():
    # Variables locales de la ventana
    win = tk.Tk()
    win.title("Agregar Cita")
    win.geometry("450x320")
    win.configure(bg="#e6f0ff")

    tk.Label(win, text="Agregar Cita", font=("Arial", 14, "bold")).pack(pady=8)

    frame = tk.Frame(win, padx=10, pady=10, bg="#e6f0ff")
    frame.pack(fill="both", expand=True)

    # Campos: código (manual), paciente (combobox), doctor (combobox), fecha, hora
    tk.Label(frame, text="Código Cita:").grid(row=0, column=0, sticky="e", pady=4)
    entry_cod_cita = tk.Entry(frame, width=30)
    entry_cod_cita.grid(row=0, column=1, pady=4)

    tk.Label(frame, text="Paciente:").grid(row=1, column=0, sticky="e", pady=4)
    ...
    tk.Label(frame, text="Doctor:").grid(row=2, column=0, sticky="e", pady=4)
    combo_doctor = ttk.Combobox(frame, values=doctores_list, width=28)
    combo_doctor.grid(row=2, column=1, pady=4)

    tk.Label(frame, text="Fecha (YYYY-MM-DD):").grid(row=3, column=0, sticky="e", pady=4)
    entry_fecha_cita = tk.Entry(frame, width=30)
    entry_fecha_cita.grid(row=3, column=1, pady=4)

    tk.Label(frame, text="Hora (HH:MM) 08:00-20:00:").grid(row=4, column=0, sticky="e", pady=4)
    ...
    def on_insert():
        ...
        try:
            cur = conn.cursor()
            # Verificar que no exista otra cita para el doctor en la misma fecha y hora
            cur.execute("""
            ...
            if cur.fetchone():
                messagebox.showerror("Conflicto horario", "El doctor ya tiene una cita a esa hora.")
                cur.close()
                conn.close()
                return
    ...
    tk.Button(frame, text="Insertar Cita", bg="#009150", fg="white", width=20, command=on_insert).grid(row=5, column=0, columnspan=2, pady=12)
